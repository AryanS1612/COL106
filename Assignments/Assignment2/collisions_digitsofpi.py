...
def Collide(Queue,M,v,x,ans,n,T,reference):
    ...
    if(time > T):
        return False
    ans.append((round(time,4),i,round(x[i]+v[i]*t,4)))
    x[i] += v[i]*t
    x[i+1] = x[i]
...
n = int(input())
print(listCollisions([100000000000000000000000,1,n],[0,1,2],[0,0,-1],100000000000000000000000000000,10000000000000000000000000000000000000))