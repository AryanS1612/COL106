...
class Node:
    ...
    def __str__(self):
        tup = (self.tuple,self.sort)
        return str(tup)

class PointDatabase:
    def __init__(self,pointlist):
        pointlists = [None]*len(pointlist)
        for i in range(len(pointlist)):# to avoid usage of tuples, which cause issue in assignment and copying.
    ...
    def segment_tree(self,lst):
        ...
        def Merge_Sort(lst,l,r):
            ...
            if(l == r):
                a = lst[l:l+1]
                node = Node((refer[l][0],refer[l][0]),a)
                return node
            else:
                ...
                Merge(lst,l,mid,r)
                a = lst[l:r+1]
                node = Node((refer[l][0],refer[r][0]),a,left,right)
                return node
